...
Deno.serve(async (req) => {
  try {
    ...
    const now = new Date();
    const requests = await base44.asServiceRole.entities.ProfessionalFamilyAccess.list('-updated_date', 500);
    ...
    for (const request of requests) {
      ...
      if (activeExpired) {
        const assignments = await base44.asServiceRole.entities.AssignedFamily.filter({ access_request_id: request.id, status: 'active' });
        for (const assignment of assignments) {
          await base44.asServiceRole.entities.AssignedFamily.update(assignment.id, { status: 'expired' });
