...
Deno.serve(async (req) => {
  try {
    ...
    const [openErrors, staleResources, unresolvedLogs, endpoints, countyConfigs] = await Promise.all([
      safeQuery(base44.asServiceRole.entities.ErrorReport.filter({ status: 'open' }, '-reported_at', 500)),
      safeQuery(base44.asServiceRole.entities.ResourceListing.filter({ verification_status: 'needs_review' }, '-updated_date', 500)),
      safeQuery(base44.asServiceRole.entities.SystemOperationalLog.filter({ resolved: false }, '-occurred_at', 500)),
      safeQuery(base44.asServiceRole.entities.IntegrationEndpoint.list('-updated_date', 200)),
      safeQuery(base44.asServiceRole.entities.StatewideCountyConfig.list('-updated_date', 200))
    ]);
    ...
    try {
      ...
      // SystemOperationalLog entity may not exist; health check still returns score.
    }
    ...
    return Response.json({ success: true, score, severity, log_id: logId });
  } catch (error) {
