...
export default function ChillicotheRossCountyHub() {
  const [resources, setResources] = useState([]);
  const [activeFilter, setActiveFilter] = useState("All");
  const [reporting, setReporting] = useState(null);

  useEffect(() => {
    base44.entities.ResourceListing.filter({ county: "Ross" }, "name", 100).then(setResources);
  }, []);

  const filtered = useMemo(() => {
    if (activeFilter === "All") return resources;
    const needle = activeFilter.toLowerCase();
    return resources.filter(resource => `${resource.category} ${resource.tags?.join(" ")} ${resource.benefit_tags?.join(" ")}`.toLowerCase().includes(needle));
  ...
  return (
    <section className="rounded-[28px] p-4 space-y-4" style={{ background: "rgba(255,255,255,0.78)", border: "1px solid rgba(255,255,255,0.75)", boxShadow: "0 18px 48px rgba(61,40,23,0.10)" }}>
      <div>
        ...
        <h2 className="font-serif font-bold text-xl mt-1" style={{ color: C.darkGreen }}>Chillicothe & Ross County Resource Hub</h2>
      ...
      <div className="space-y-3">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[11px] mt-1" style={{ color: C.mutedText }}>{resource.city || "Chillicothe"}, {resource.county} County · {statusLabel(resource.verification_status)}</p>
                  </div>
        ...
        {filtered.length === 0 && <p className="rounded-2xl p-4 text-center text-sm" style={{ background: C.cream, color: C.mutedText }}>No Ross County resources match this filter yet.</p>}
      </div>
