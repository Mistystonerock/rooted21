...
const CARD = SURFACES.darkCard;
const BORDER = SURFACES.darkCardBorder;
const GOLD = TEXT.accentSecondary;
const GREEN = TEXT.accent;
const TEXT_COLOR = TEXT.onDark;
const MUTED = TEXT.mutedDark;

const QUICK_ACTIONS = [
  { to: "/chat?crisis=1", icon: "SOS", label: "Help Me Right Now", sub: "Get immediate support", border: CARD_COLORS.sos.border, textColor: CARD_COLORS.sos.border, issos: true },
  { to: "/daily-checkin", icon: "✓", label: "Check-In", sub: "Daily check-in & mood", border: CARD_COLORS.checkin.border, textColor: CARD_COLORS.checkin.border },
  { to: "/behavior-logs", icon: "📊", label: "Log Behavior", sub: "Track & understand behaviors", border: CARD_COLORS.behavior.border, textColor: CARD_COLORS.behavior.border },
  { to: "/safety-plan", icon: "🛡", label: "Safety Plan", sub: "Crisis plan & resources", border: CARD_COLORS.safety.border, textColor: CARD_COLORS.safety.border },
];
...
export default function HomeScreen({ onHelp, error, onOpenHistory, onOpenTrends }) {
  ...
  return (
    <div style={{ background: BG, minHeight: "100vh", fontFamily: "var(--font-sans)", color: TEXT_COLOR, paddingBottom: 90 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "max(16px, env(safe-area-inset-top)) 16px 12px", background: BG, position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <TreeLogo size={32} />
          <div>
            <p style={{ fontFamily: "var(--font-serif)", fontWeight: 800, fontSize: 18, color: TEXT_COLOR, lineHeight: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={() => navigate("/chat")} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <MessageCircle size={18} color={TEXT_COLOR} />
          </button>
          <button onClick={() => navigate("/notifications")} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
            <Bell size={18} color={TEXT_COLOR} />
            <span style={{ position: "absolute", top: 6, right: 6, width: 8, height: 8, borderRadius: "50%", background: "#e05555", border: `1.5px solid ${BG}` }} />
          </button>
          <button onClick={() => navigate("/profile")} style={{ background: CARD, border: `1.5px solid ${GREEN}40`, borderRadius: "50%", width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontWeight: 800, fontSize: 15, color: TEXT_COLOR }}>
            {user?.full_name?.[0] || "?"}
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: "0 16px", maxWidth: 520, margin: "0 auto" }}>
        ...
        {/* ── WELCOME CARD ── */}
        <div style={{ background: CARD, border: `1.5px solid ${BORDER}`, borderRadius: 16, padding: "18px 18px 16px", marginBottom: 22, marginTop: 6, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            <p style={{ fontFamily: "var(--font-serif)", fontWeight: 800, fontSize: 20, color: TEXT_COLOR, lineHeight: 1.2 }}>
              Welcome back, {firstName} 🌿
            </p>
            <p style={{ fontSize: 13, color: MUTED, marginTop: 5 }}>You are not alone in this journey.</p>
          </div>
          <Link to="/personalized-chat" style={{ background: CARD, border: `1.5px solid ${GREEN}60`, borderRadius: 10, padding: "9px 14px", display: "flex", alignItems: "center", gap: 6, textDecoration: "none", flexShrink: 0 }}>
        ...
        {/* ── QUICK ACTIONS ── */}
        <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.16em", color: MUTED, marginBottom: 10, textTransform: "uppercase" }}>Quick Actions</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 22 }}>
          {QUICK_ACTIONS.map(a => (
            <Link key={a.to} to={a.to} style={{ background: CARD, border: `2px solid ${a.border}`, borderRadius: 14, padding: "12px 8px", textDecoration: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textAlign: "center" }}>
              {a.issos ? (
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "transparent", display: "flex", alignItems: "center", justifyContent: "center", border: `2px solid ${a.border}`, fontWeight: 900, fontSize: 10, color: a.border }}>SOS</div>
              ) : a.icon.length <= 2 && /[a-zA-Z✓]/.test(a.icon) ? (
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "transparent", display: "flex", alignItems: "center", justifyContent: "center", border: `2px solid ${a.border}`, fontSize: 18, color: a.textColor, fontWeight: 800 }}>{a.icon}</div>
              ) : (
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{a.icon}</div>
              )}
              <p style={{ fontSize: 11, fontWeight: 800, color: TEXT_COLOR, lineHeight: 1.2 }}>{a.label}</p>
        ...
        {/* ── YOUR PROGRESS ── */}
        <div style={{ background: CARD, border: `1.5px solid ${BORDER}`, borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.14em", color: MUTED, marginBottom: 14, textTransform: "uppercase" }}>Your Progress</p>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ position: "relative", width: 88, height: 88, flexShrink: 0 }}>
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <p style={{ fontWeight: 900, fontSize: 18, color: TEXT_COLOR, lineHeight: 1 }}>{progressPct}%</p>
              ...
              <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
                {[
                  ...
                  <div key={s.label} style={{ textAlign: "center", flex: 1 }}>
                     <p style={{ fontWeight: 900, fontSize: 22, color: TEXT_COLOR, lineHeight: 1 }}>{s.val}</p>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.16em", color: MUTED, textTransform: "uppercase" }}>Features</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 22 }}>
          {FEATURES.map(f => (
            <Link key={f.to} to={f.to} style={{ background: CARD, border: `1.5px solid ${BORDER}`, borderRadius: 14, padding: "12px 10px", textDecoration: "none", display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <p style={{ fontSize: 11, fontWeight: 700, color: TEXT_COLOR, lineHeight: 1.2 }}>{f.label}</p>
        ...
        <div style={{ background: CARD, border: `1.5px solid ${GREEN}40`, borderRadius: 16, padding: "16px 18px", display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
              <p style={{ fontWeight: 800, fontSize: 14, color: TEXT_COLOR }}>AI Parenting Support</p>
        ...
        {error && <p style={{ fontSize: 12, color: TEXT.danger, textAlign: "center", marginTop: 8 }}>⚠️ {error}</p>}

      </div>

      {/* ── BOTTOM NAV ── */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: BG, borderTop: `1px solid ${BORDER}`, paddingBottom: "max(12px, env(safe-area-inset-bottom))", zIndex: 20, display: "flex", alignItems: "center" }}>
        {[
        ...
        <div style={{ flex: 1, display: "flex", justifyContent: "center", padding: "4px 0" }}>
          <Link to="/chat?crisis=1" style={{ width: 54, height: 54, borderRadius: "50%", background: CARD_COLORS.sos.border, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textDecoration: "none", boxShadow: "0 4px 16px rgba(231,76,60,0.4)", border: `3px solid ${BG}`, marginTop: -16 }}>
