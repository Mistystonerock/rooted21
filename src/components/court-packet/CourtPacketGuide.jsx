...
import { CalendarPlus, FileText, FolderOpen, MessageCircle, Printer, ShieldAlert } from "lucide-react";
...
export default function CourtPacketGuide({ packet, user }) {
  ...
  return (
    <div className="space-y-4 print:bg-white">
      ...
      <Section title="2. Important safety/legal warning" defaultOpen><p>{packet.warning}</p></Section>
      <Section title="3. Official forms and where to get them"><BulletList items={packet.forms} /></Section>
      <Section title="4. Documents to gather"><BulletList items={packet.documents} /></Section>
      <Section title="5. Questions to ask the court clerk or legal aid"><BulletList items={packet.questions} /></Section>
      <Section title="6–8. Fees, fee waiver, and service reminders"><p>{packet.fees}</p><p className="mt-3">{packet.service}</p></Section>
      <Section title="9. Hearing preparation checklist"><BulletList items={packet.hearing} /></Section>
      <Section title="10. Evidence/document checklist"><BulletList items={packet.evidence} /></Section>

      <Section title="11–12. Deadline tracker and court date reminder" defaultOpen>
      ...
      <Section title="13. Notes section" defaultOpen><textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Write calm notes, questions, deadlines, or next steps here..." className="min-h-32 w-full rounded-2xl border p-3 text-sm" style={{ borderColor: C.cream }} /></Section>

      <Section title="14. Document Vault folders and uploads" defaultOpen>
      ...
      <Section title="15–17. Print, ask Moxie, and connect to official resources" defaultOpen>
