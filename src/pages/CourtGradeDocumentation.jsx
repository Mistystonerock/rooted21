...
import HearingTimelinePanel from "@/components/court-records/HearingTimelinePanel";
import CasePlanCourtTracker from "@/components/court-records/CasePlanCourtTracker";
import VisitationCourtTracker from "@/components/court-records/VisitationCourtTracker";
import CourtExportButton from "@/components/court-records/CourtExportButton";
      ...
      <main className="mx-auto max-w-5xl space-y-5 px-4 py-5 pb-28 print:max-w-none print:pb-4">
        <section className="rounded-3xl bg-green-900 p-5 text-white shadow-sm print:bg-white print:text-stone-900 print:border">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-white/80 print:text-stone-600">Each submitted record receives a timestamp, authentication ID, verification ID, hash-chain record, and cannot be edited or deleted after submission.</p>
            </div>
            <div className="flex flex-wrap gap-2 print:hidden">
              ...
              <CourtExportButton caseNumber={cases[0]?.case_number} />
            </div>
          </div>
        </section>
        ...
        {loading ? <div className="rounded-2xl bg-white p-6 text-center">Loading court records...</div> : (
          <>
            ...
            <HearingTimelinePanel caseSnapshot={cases[0]} />
            <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {deadlines.map(deadline => <CourtDeadlineCard key={deadline.title} deadline={deadline} />)}
            </section>
            <div className="grid gap-4 lg:grid-cols-2">
              <VisitationCourtTracker visits={visits} />
              <CasePlanCourtTracker checklists={checklists} />
            </div>
