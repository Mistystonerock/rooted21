    ...
    <div className="min-h-screen" style={{ background: BG, color: DARK }}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        ...
        <FounderSection title="Waitlist Management" subtitle="Review waitlist signups and invite people into the app" icon={MailPlus} open={openSections.waitlist} onToggle={() => toggle("waitlist")}>
          <div className="grid md:grid-cols-2 gap-3">{waitlist.map(w => <div key={w.id} className="rounded-2xl p-4 border" style={{ borderColor: "#d7c7aa", background: "#faf6f1" }}><div className="flex justify-between gap-3"><div><p className="font-bold">{w.full_name}</p><p className="text-xs" style={{ color: MUTED }}>{w.email}</p></div><span className="text-[10px] font-bold">{w.notified_at_launch ? "Invited" : "Waiting"}</span></div><p className="text-xs mt-2" style={{ color: MUTED }}>{w.family_type || "—"} {w.city ? `· ${w.city}` : ""}</p>{w.message && <p className="text-xs mt-2 italic">“{w.message}”</p>}<div className="mt-3">{smallButton(w.notified_at_launch ? "Invite again" : "Invite", () => inviteWaitlistPerson(w))}</div></div>)}</div>
        </FounderSection>
        ...
        <FounderSection title="Grant and Donation Tracker" subtitle="Nonprofit grant, donation, sponsorship, and expense logging" icon={DollarSign} open={openSections.funding} onToggle={() => toggle("funding")}>
