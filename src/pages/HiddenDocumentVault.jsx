...
export default function HiddenDocumentVault() {
  ...
  async function logAction(action, summary, evidence_id = "") {
    if (!user) return;
    const log = await base44.entities.SurvivorVaultAuditLog.create({ owner_email: user.email, evidence_id, action, summary, occurred_at: new Date().toISOString() });
  ...
  async function exportManifest() {
    ...
    a.href = url;
    a.download = `survivor-vault-manifest-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
    await logAction("secure_export", "Secure vault manifest exported");
  }

  const filteredItems = useMemo(() => filter === "all" ? items : items.filter(item => item.category === filter), [items, filter]);
  ...
  return (
    <main className="min-h-screen pb-28" style={{ background: C.offWhite }}>
      ...
      <div className="mx-auto grid max-w-[720px] gap-4 px-4 py-5 lg:grid-cols-[280px_1fr]">
        ...
        <div className="space-y-4">
          <select value={filter} onChange={e => setFilter(e.target.value)} className="w-full rounded-2xl px-3 py-3 text-sm font-bold" style={{ border: `1px solid ${C.cream}`, background: C.white }}>
          ...
          {filteredItems.length === 0 && <div className="rounded-3xl p-6 text-center text-sm font-bold" style={{ background: C.white, color: C.mutedText, border: `1px solid ${C.cream}` }}>No vault items yet.</div>}

          <section className="rounded-3xl p-4 shadow-sm" style={{ background: C.white, border: `1px solid ${C.cream}` }}>
            <h2 className="mb-3 font-serif text-lg font-black" style={{ color: C.darkGreen }}>Append-only log</h2>
            <div className="space-y-2">
              {logs.slice(0, 8).map(log => <p key={log.id} className="rounded-xl p-3 text-xs" style={{ background: C.offWhite, color: C.mutedText }}><strong>{new Date(log.occurred_at).toLocaleString()}:</strong> {log.summary}</p>)}
            </div>
          </section>
        </div>
      </div>
