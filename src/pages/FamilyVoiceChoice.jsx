    ...
    <main className="min-h-screen" style={{ background: C.offWhite }}>
      ...
      <div className="mx-auto max-w-[520px] space-y-4 px-4 py-5">
        ...
        <section className="rounded-2xl p-4 space-y-3" style={{ background: C.white, border: `1.5px solid ${C.cream}` }}>
          ...
          <div><p className="text-xs font-black" style={{ color: C.darkGreen }}>Choose and rank preferred services</p><div className="mt-2 flex flex-wrap gap-2">{SERVICES.map(service => <button key={service} onClick={() => toggleService(service)} className="rounded-full px-3 py-1.5 text-[11px] font-black" style={{ background: form.ranked_services.includes(service) ? C.darkGreen : C.offWhite, color: form.ranked_services.includes(service) ? C.white : C.darkGreen, border: `1px solid ${C.cream}` }}>{form.ranked_services.includes(service) ? `${form.ranked_services.indexOf(service) + 1}. ` : ''}{service}</button>)}</div></div>
          <button onClick={save} className="w-full rounded-2xl py-3 text-sm font-black" style={{ background: saved ? C.midGreen : C.darkGreen, color: C.white, border: "none" }}>{saved ? "Saved" : "Save Family Preferences"}</button>
        </section>
