  ...
  const filteredResources = useMemo(() => {
    ...
    return resources.filter(resource => {
      const haystack = normalize([
        resource.name, resource.city, resource.county, resource.cost, resource.activity_type, resource.eligibility_notes, resource.setting,
        ...(resource.tags || []), ...(resource.benefit_tags || [])
      ].join(" "));
      ...
      <main className="mx-auto max-w-[720px] space-y-5 px-4 py-5 pb-36">
        ...
        <section className="space-y-3">
          <div className="relative">
            ...
            <input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search ZIP, county, city, free, EBT, indoor, outdoor, teen, sensory-friendly…" className="w-full rounded-2xl border py-3 pl-10 pr-3 text-sm" style={{ borderColor: C.cream }} />
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border p-6 text-center" style={{ background: C.white, borderColor: C.cream }}>
            <p className="font-bold" style={{ color: C.darkGreen }}>No matching activities found yet.</p>
            <p className="mt-1 text-xs" style={{ color: C.mutedText }}>Try clearing filters or searching by county, city, park, library, EBT, indoor, outdoor, or age range.</p>
          </div>
        ) : (
